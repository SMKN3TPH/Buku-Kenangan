...
// EMOJI_REACTIONS dihapus — gunakan EMOJI_LIST dari utils.js
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    bindCharCount();
    setupFilterButtons();
    // loadPesan dipanggil dari pesan.html
    // Profil modal hanya muncul saat user klik kirim (bukan saat halaman dibuka)
});
    ...
    if (!messages.length) {
        container.innerHTML = `
            <div style="text-align:center;padding:48px 24px;color:var(--text-muted)">
                <div style="font-family:'Playfair Display',serif;font-size:15px;font-style:italic;
                            color:var(--text-dim);line-height:2;margin-bottom:16px">
                    "Belum ada yang menulis di sini.<br>Mungkin kamu bisa jadi yang pertama."
                </div>
                <div style="width:32px;height:1px;background:var(--border-gold);margin:0 auto"></div>
            </div>`;
        return;
    }
...
function buildCard(m) {
    ...
    const metaLabel = m.peran || (m.jurusan ? getJurusanLabel(m.jurusan) : '');

    return `
        <div class="message-card" id="msg-card-${m.id}">
            <div class="message-header">
                <div class="msg-identity">
                    <div class="msg-avatar">${escHtml(m.nama.charAt(0).toUpperCase())}</div>
                    <div>
                        <div class="message-author">${escHtml(m.nama)}</div>
                        <div class="message-meta">${escHtml(metaLabel)}</div>
                    </div>
                </div>
...
async function submitMessage() {
    ...
    if (!profile) {
        // Pertama kali kirim — minta isi profil dulu, lalu lanjut kirim
        ProfileModal.show((newUser) => {
            // Tampilkan sender chip setelah profil diisi
            const chip = document.getElementById('sender-info');
            if (chip) {
                const jurusanDisplay = newUser.jurusan ? ' · ' + getJurusanLabel(newUser.jurusan) : '';
                const span = chip.querySelector('span');
                if (span) span.textContent = `${newUser.name} · ${newUser.role}${jurusanDisplay}`;
                chip.classList.add('visible');
            }
            injectProfileChip(newUser);
            // Langsung submit setelah profil tersimpan
            submitMessage();
        });
        return;
    }
...
async function loadPesanConfig() {
    ...
    const value = await fetchSiteConfig('pesan_close_date');
    if (value) {
        _pesanCloseDate = new Date(value);
